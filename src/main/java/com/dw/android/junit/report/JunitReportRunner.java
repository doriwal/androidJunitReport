package com.dw.android.junit.report;
 ...
 * This is the main class which will be invoked by android instrumentation , in this class i use a custom junit listener
 * In order to listen to the Junit events and to print them to a file in the device
 * code is a fork from  http://zutubi.com/source/projects/android-junit-report/
 */
public class JunitReportRunner extends MonitoringInstrumentation {

    private static final String LOG_TAG = "JunitReportRunner";
    ...
    private void send(){
        ...
        intent.setAction("junit.tests.finished");
        intent.putExtra("test.package", mArguments.getString("test.package"));
    ...
    public void finish(int resultCode, Bundle results) {
        try {
            ...
            UsageTrackerRegistry.getInstance().trackUsage("JunitReportRunner");
    ...
    private void addListeners(RunnerArgs args, TestExecutor.Builder builder) {
        if (args.suiteAssignment) {
            ...
            this.mListener = new JUnitReportListener(getTargetContext(),
                    "junit.xml",
                    Environment.getExternalStorageDirectory().getAbsolutePath() + "/tests_results/",
                    false,
                    true);
